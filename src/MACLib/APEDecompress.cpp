...
CAPEDecompress::CAPEDecompress(int * pErrorCode, CAPEInfo * pAPEInfo, int nStartBlock, int nFinishBlock)
{
    *pErrorCode = ERROR_SUCCESS;
    ...
    m_nStartBlock = (nStartBlock < 0) ? 0 : (std::min<long int>)(nStartBlock, GetInfo(APE_INFO_TOTAL_BLOCKS));
    m_nFinishBlock = (nFinishBlock < 0) ? GetInfo(APE_INFO_TOTAL_BLOCKS) : (std::min<long int>)(nFinishBlock, GetInfo(APE_INFO_TOTAL_BLOCKS));
...
int CAPEDecompress::GetData(char * pBuffer, int nBlocks, int * pBlocksRetrieved)
{
    ...
    const int nBlocksToRetrieve = (std::min<long int>)(nBlocks, nBlocksUntilFinish);
    ...
    while ((nBlocksLeft > 0) && (nBlocksThisPass > 0))
    {
        ...
        nBlocksThisPass = (std::min<long int>)(nBlocksLeft, nFrameBufferBlocks);
...
int CAPEDecompress::FillFrameBuffer()
{
    ...
    while (nBlocksLeft > 0)
    {
        ...
        {
            // output silence
            int nOutputSilenceBlocks = (std::min<long int>)(m_nErrorDecodingCurrentFrameOutputSilenceBlocks, nBlocksLeft);
        ...
        int nBlocksThisPass = (std::min<long int>)(nFrameBlocksLeft, nBlocksLeft);
